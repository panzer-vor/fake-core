...
const PromotionDetailComponent: React.SFC<Props> = ({
    ...
    const [validFailed, setValidFailed] = useState(false);
    useEffect(() => {
    ...
    const ValidRules = {
        code: [{type: "string", required: true, pattern: /^[a-zA-Z0-9\$\%]{4,15}$/, message: "Required, at least 4 characters. at most 15 characters, allow: a-z, A-Z, 0-9, $, %."}],
        name: [{type: "string", required: true, min: 4, max: 22}],
        // description: [{required: true, message: "Description (Terms & Conditions) is required."}],
    ...
    const handleSubmit = (event: React.FormEvent<any> | null, status?: PromotionStatusAJAXView) => {
        ...
        validateFields((err, values) => {
            ...
            if (!err && !validFailed) {
                ...
                const request = {
                    name: values.name,
                    description: draftToHtml(values.description) ? draftToHtml(values.description) : values.description,
    ...
    const checkEditorValid = () => {
        const data = getFieldValue("description");
        if (data || promotionDetail.description) {
            if (data.blocks && data.blocks.length === 1 && !data.blocks[0].text) {
                setValidFailed(true);
                return true;
            }
    ...
    return (
        <div className="promotion-detail-outer">
                        ...
                        <Row gutter={20}>
                            <Col span={12}>
                                <Row gutter={20} style={{marginLeft: 0, marginRight: 0}}>
                                    <Form.Item label="Promotion Code" labelCol={{span: 10}} style={{marginBottom: 0}}>
                                        {getFieldDecorator("code", {initialValue: promotionDetail.code, rules: ValidRules.code})(<Input disabled={!!promotion_code} />)}
                                    </Form.Item>
                                    <Form.Item label="Promotion Name" labelCol={{span: 10}} style={{marginBottom: 0}}>
                                        {getFieldDecorator("name", {initialValue: promotionDetail.name, rules: ValidRules.name})(<Input />)}
                                    </Form.Item>
                                    <Form.Item label="Brief Description" labelCol={{span: 10}} style={{marginBottom: 0}}>
                                        {getFieldDecorator("brief_description", {initialValue: promotionDetail.brief_description, rules: ValidRules.brief_description})(<Input />)}
                                    </Form.Item>
                                    ...
                                    <Col span={12}>
                                        <Form.Item style={{marginBottom: 0}} label="Distribution Start Date">
                                            ...
                                            })(<DatePicker placeholder="" disabled={isActiveEdit} format={DateFormat.DATE_UTC} style={{width: "100%"}} />)}
                                        </Form.Item>
                                    </Col>
                                    <Col span={12}>
                                        <Form.Item style={{marginBottom: 0}} label="Distribution End Date">
                                            ...
                                            })(<DatePicker placeholder="" format={DateFormat.DATE_UTC} style={{width: "100%"}} />)}
                                        </Form.Item>
                                    </Col>
                                    {/* </div> */}
                                    {/* <Row gutter={20} className="promotion-list_search-form promotion-list_form"> */}
                                    <Col span={12}>
                                        <Form.Item style={{marginBottom: 0}} label="Valid Start Date">
                                    ...
                                    <Col span={12}>
                                        <Form.Item style={{marginBottom: 0}} label="Valid End Date">
                            ...
                            <Col span={12} style={{marginTop: 0}}>
                                <Form.Item className="promo-desc" label="Description (Terms & Conditions)" labelCol={{span: 10}}>
                                    <HtmlEditor validFailed={validFailed} decorator={getFieldDecorator("description", {initialValue: promotionDetail.description})} data={promotionDetail.description || getFieldValue("description")} />
                        ...
                        <Row>
                            <Form.Item className="promotion-list_search-form promotion-list_form" style={{paddingBottom: "24px", marginBottom: 0}}>
                                {getFieldDecorator("eligibility", {
                                    ...
                                    <Radio.Group disabled={isActiveEdit}>
                                        <Row type="flex">
                            ...
                            <Row className="promotion-list_search-form promotion-list_form" gutter={10}>
                                            ...
                                            return (
                                                <Col span={6}>
                                                    <Form.Item label={PROMOTION_TYPE[PromotionTypeAJAXView.AMOUNT]} labelCol={{span: 10}} className="promotion-list_form-field promotion-purchase-condition">
                                                        {getFieldDecorator("amount_off", {initialValue: promotionDetail.amount_off, rules: ValidRules.amount})(<Input addonBefore="$" disabled={isActiveEdit} />)}
                                                    </Form.Item>
                                                </Col>
                                            );
                                                ...
                                                <>
                                                    <Col span={6}>
                                                        <Form.Item label={PROMOTION_TYPE[PromotionTypeAJAXView.PERCENTAGE]} labelCol={{span: 8}} className="promotion-list_form-field">
                                                            {getFieldDecorator("percentage_off", {initialValue: promotionDetail.percentage_off, rules: ValidRules.percentage})(<Input addonAfter="%" disabled={isActiveEdit} />)}
                                                        </Form.Item>
                                                    </Col>
                                                    <Col span={6}>
                                                        <Form.Item label="Max $ discount each time" labelCol={{span: 10}} className="promotion-list_form-field promotion-purchase-condition">
                                                            {getFieldDecorator("max_discount", {initialValue: promotionDetail.max_discount, rules: ValidRules.max_discount})(<Input addonBefore="$" disabled={isActiveEdit} />)}
                                                        </Form.Item>
                                                    </Col>
                                                </>
                                            );
                                ...
                                <Col span={6}>
                                    <Form.Item label="Purchase at least" labelCol={{span: 10}} className="promotion-list_form-field promotion-purchase-condition">
                                        {getFieldDecorator("purchase_minimum", {initialValue: promotionDetail.purchase_minimum, rules: ValidRules.minimum})(<Input addonBefore="$" style={{width: "191px"}} disabled={isActiveEdit} />)}
                                    </Form.Item>
                                </Col>
                            </Row>
                        </Col>
                        ...
                        <Row>
                            <Col span={6} style={{marginLeft: 0}}>
                                <Form.Item label="Max # of use Times" labelCol={{span: 10}} className="promotion-list_form-field promotion-list_search-form promotion-list_form" style={{paddingBottom: "24px", marginBottom: 0, paddingRight: "5px"}}>
                                    ...
                                    })(<Input placeholder="Empty means no limit" />)}
                                </Form.Item>
                            </Col>
                        </Row>
                    </Row>
...
export default connect(mapStateToProps, mapDispatchToProps)(Form.create()(PromotionDetailComponent));
