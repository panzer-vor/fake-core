...
class Main extends React.Component<Props & RouteProps> {
    hasPermission(roles: string[] | null): boolean {
        return Boolean(roles && (roles.indexOf("Customer") >= 0 || roles.indexOf("Marketing") >= 0));
    }
        ...
        return (
            <>
                ...
                <Layout className="fti">
                    ...
                    <Layout>
                        <Header />
                        <Route exact key={"/nonexistence"} path={"/nonexistence"} component={() => <Nonexistence />} />
                        <Route exact key={"/"} path={"/"} component={() => <Redirect to="/restaurant-list-ads" />} />
                        <Layout.Content className="fti-layout">
                            <div className="fti-layout__content">
                                <div key={location && location.pathname}>
                                    <Switch location={location}>{Config.routes.map(route => this.renderRoute(route))}</Switch>
                                </div>
                            </div>
                        </Layout.Content>
                    </Layout>
...
export const Nonexistence = () => (
    <div style={{padding: "32px"}}>
        <h1>
            Your account doesn't exist, please contact Administrator to create an account for you, or <a href="/logout">Logout</a> and then try another account.
        </h1>
    </div>
);
